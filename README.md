...
“硬件在环报告”页会从真实 MCU 日志中识别 OTA 验签、Trial 确认、下载/安装掉电恢复、Trial 回退与 Factory 制备/恢复证据，并一键导出 `reports/hil-report-*.md`。每个完整通过场景会自动写入 `reports/hil-evidence.json`；下次打开上位机仍会加载，便于从“待验证”项继续。它不会模拟硬件动作：三个掉电场景仍须人工断电，缺少完整恢复日志会保持为“待验证”。

...
上位机“硬件在环报告”页读取真实 MCU UART 输出，并可导出 `reports/hil-report-*.md`。每个完整通过项会自动保存到 `reports/hil-evidence.json`；下次启动会显示“历史保存证据”，因此可从仍为“待验证”的项目继续。可在上位机中清除该台账后重新开始。它把以下场景明确分为“通过”或“待验证”：安全 OTA 验签、Trial 确认、下载阶段恢复、安装阶段恢复、Trial 未确认回退、Factory 制备和 Factory 恢复。
